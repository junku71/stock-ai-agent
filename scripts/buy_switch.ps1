<#
.SYNOPSIS
    신규 매수 원격 on/off — Windows(PowerShell) 용 래퍼.

.DESCRIPTION
    buy_switch.sh 의 PowerShell 판. Windows 에서 bash/curl.exe 를 거치지 않고
    ...
    Git Bash(LANG=ko_KR.UTF-8)에서는 buy_switch.sh 를 그대로 써도 정상이다.

    ※ 참고 — 과거 사유가 "???" 로 저장된 적이 있는데, 위 두 경로 모두 이 환경에서
      재현되지 않았다(셋 다 정상 UTF-8 전송 확인). 원인 미상이므로 사유가 물음표로만
      남아 있으면 그대로 믿지 말고 다시 설정할 것.
...
.EXAMPLE
    .\scripts\buy_switch.ps1 status
.EXAMPLE
    .\scripts\buy_switch.ps1 off "출장 - 8/25 복귀"
.EXAMPLE
    .\scripts\buy_switch.ps1 on
.NOTES
    서버 주소는 BUY_SWITCH_HOST 환경변수로 바꾼다 (기본 http://localhost:8000).
#>
[CmdletBinding()]
param(
...
$Host_ = if ($env:BUY_SWITCH_HOST) { $env:BUY_SWITCH_HOST } else { 'http://localhost:8000' }
...
if ($Action -eq 'status') {
    try {
        Show-Result (Invoke-RestMethod -Method Get -Uri "$Host_/buy-switch")
    }
    catch {
...
if ($Action -eq 'off') {
    ...
    $uri = "$Host_/buy-switch/disable?reason=$encoded"
}
else {
    $uri = "$Host_/buy-switch/enable"
}
